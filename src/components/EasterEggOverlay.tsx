...
export const EasterEggOverlay = memo(function EasterEggOverlay({ show }: EasterEggOverlayProps) {
    ...
    return (
        <div aria-hidden="true" className="fixed inset-0 z-50 pointer-events-none flex items-center justify-center overflow-hidden">
            {/* Vignette */}
            <div
                className={`absolute inset-0 transition-opacity duration-500 ${animate ? 'opacity-100' : 'opacity-0'}`}
                style={{
            ...
            {/* Calendar Container */}
            <div
                className="relative w-64 h-80 bg-white rounded-lg shadow-2xl overflow-hidden transform scale-150 md:scale-100 transition-opacity duration-500"
                style={{
                    boxShadow: '0 20px 50px rgba(0,0,0,0.5)',
                    opacity: animate ? 1 : 0,
                }}
            >
                {/* Horse Page (Bottom — revealed) */}
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#fdfbf7] text-[#d93025]">
                    ...
                    <div className="absolute bottom-4 text-xs text-gray-500 tracking-widest">YEAR OF THE HORSE</div>
                </div>

                {/* Snake Page (Top — tears off) */}
                {animate && (
                    <div
                        className="absolute inset-0 flex flex-col items-center justify-center bg-[#fdfbf7] text-[#2d5a27] origin-top-left animate-calendar-tear"
                        style={{
                    ...
                    >
                        ...
                        <div className="absolute bottom-4 text-xs text-gray-500 tracking-widest">YEAR OF THE SNAKE</div>
